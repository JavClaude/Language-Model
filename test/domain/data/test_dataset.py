...
@patch("language_model.domain.modeling.data.dataset.LanguageModelingDataset._check_is_fitted")
@patch("language_model.domain.modeling.data.dataset.LanguageModelingDataset._fit_tokenizer")
...
@patch("language_model.domain.modeling.data.dataset.LanguageModelingDataset._preprocess", return_value=(1, 2))
...
def test_language_model_dataset_fit_tokenizer_should_call_the_train_method_of_bpe_tokenizer():
    ...
    # Then
    tokenizer.train.assert_called_with(FAKE_PATH_FOR_TEST, 300, special_tokens=[EOD_TOKEN, PAD_TOKEN, SOS_TOKEN, UNK_TOKEN])
...
def test_language_model_dataset_get_total_number_of_batches_should_return_correct_sequence_of_ids_length():
    ...
    # When
    output = language_modeling_dataset._get_total_number_of_batches(sequence_length, 5, 2)
...
def test_language_model_dataset_truncate_sequence_of_ids_for_batch_processing_should_return_correct_output():
    ...
    # When
    output = language_modeling_dataset._truncate_sequence_of_ids_for_batch_processing(sequence_of_ids, batch_size, bptt, total_number_of_batches)

    # Then
    assert output == sequence_of_ids[0: 100]


@patch("language_model.domain.modeling.data.dataset.np.reshape")
def test_language_model_dataset_reshape_sequence_of_ids_for_batch_processing_should_call_reshape_function(
    ...
    # When
    _ = language_modeling_dataset._reshape_sequence_of_ids_for_batch_processing(sequence_of_ids, batch_size)

    # Then
    reshape_mock.assert_called_with(sequence_of_ids, (batch_size, -1))


@patch("language_model.domain.modeling.data.dataset.np.zeros_like")
