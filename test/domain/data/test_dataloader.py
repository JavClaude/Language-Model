...
def test_language_modeling_dataloader_check_if_sequence_of_ids_is_a_tuple_should_return_true_when_sequence_of_ids_is_a_tuple():
    ...
    # When
    output = language_modeling_dataloader._check_if_sequence_of_ids_is_a_tuple(sequence_of_ids)
...
def test_language_modeling_dataloader_check_sequence_of_ids_length_should_raise_exception_for_a_3_element_tuple():
    ...
    with pytest.raises(ValueError) as exc_info:
        _ = language_modeling_dataloader._check_if_sequence_of_ids_is_a_tuple(sequence_of_ids)
...
def test_language_modeling_dataloader_get_batches_should_return_a_tuple_of_tensor_when_sequence_of_ids_is_a_tuple():
    ...
    sequence_of_ids = (
        train_sequence_of_ids,
        target_sequence_of_ids
    )
    ...
    # Then
    assert equal(output[0], tensor(train_sequence_of_ids[:, batch_index: batch_index + bptt], device=DEVICE, dtype=long))
    assert equal(output[1], tensor(target_sequence_of_ids[:, batch_index: batch_index + bptt], device=DEVICE, dtype=long))
...
def test_language_modeling_dataloader_get_batches_should_return_a_tensor_when_sequence_of_ids_is_a_ndarray():
    ...
    # Then
    assert equal(next(output), tensor(sequence_of_ids[:, batch_index: batch_index + bptt], device=DEVICE, dtype=long))
